...
impl YOLOv8 {
    pub fn new(onnxt: String, conf: f32, device_type: OrtEP) -> Result<Self> {
        // execution provider
        // using TensorRT EP
        // OrtEP::Trt(config.device_id)
        //     OrtEP::Cuda(config.device_id)
        //     OrtEP::CPU(config.device_id)
        let ep = device_type;
        // let ep = OrtEP::Cuda(1);
        ...
        let ort_args = OrtConfig {
            ...
            batch,
            f: onnxt, //模型地址文件地址
            task: Some(YOLOTask::Detect),//Option
            trt_fp16: false, // 默认false TensorRT EP 使用fp16推理
        ...
        Ok(Self {
            engine,
            names,
            conf: conf, // confidence threshold 信任度
            kconf: 0.55, //onfidence threshold of keypoint  你的 YOLO 模型需要支持关键点检测任务，例如，检测人体的关键点（头部、手肘、膝盖等）
            //较高的 iou (例如，0.7) 会使 NMS 更严格，只保留重叠程度非常低的边界框，可能会导致漏检。
            // 较低的 iou (例如，0.4) 会使 NMS 更宽松，保留更多边界框，但可能会增加误检的可能性。
